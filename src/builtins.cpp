...
void type(std::string command) {
        if (std::find(BUILTINS.begin(), BUILTINS.end(), command) !=
            BUILTINS.end()) {
...
void exec(const std::vector<std::string>& args, const inputMetaData& meta) {
        ...
        if (pid == -1) {
                ...
                if (errno == ENOENT) {
                        printErr(/*"jsh: " + */ std::string(argv[0]) +
                                     ": command not found",
                                 true); // commented the jsh: out for now, to
                                        // pass tests
                        _exit(127);
...
void pwd() { print(std::filesystem::current_path().string(), true); }
...
void cd(const std::string& newPath) {
        ...
        try {
                ...
                printErr("cd: " + newPath + ": No such file or directory",
                         true);
        }
}

void toggleCwd() { showCwd = !showCwd; }
